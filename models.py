...
class PerceptronModel(object):
    ...
    def get_prediction(self, x):
        ...
        Returns: 1 or -1
        """
        product = nn.DotProduct(self.w, x)
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        second_layer = nn.AddBias(nn.Linear(first_layer, self.w1), self.b1)
        return second_layer
    ...
    def train(self, dataset):
        ...
        while accuracy < 0.98:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                self.b1.update(gradients[3], -self.alpha)
                self.w2.update(gradients[4], -self.alpha)
                self.b2.update(gradients[5], -self.alpha)
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        for i in xs[1:]:
            h = nn.Add(nn.Linear(i, self.w0), nn.Linear(h, self.w1))
            h = nn.ReLU(nn.AddBias(h, self.b1))
        output_layer = nn.AddBias(nn.Linear(h, self.w2), self.b2)
        return output_layer
